...
import {createPlanet} from "../helpers/planets.ts";
...
export async function initialize(canvas: HTMLCanvasElement) {
    ...
    const moonFragmentShader = await loadTextResource("/shaders/sun.frag") as string;
    ...
    const sun = createPlanet(gl, sunShader, {radius: 5, latitudeBands: 30, longitudeBands: 30});
    objects.push(sun);

    const moon = createPlanet(gl, moonShader, {radius: 2, latitudeBands: 30, longitudeBands: 30});
    objects.push(moon);
